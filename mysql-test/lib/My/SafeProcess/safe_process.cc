...
int main(int argc, char *const argv[]) {
  ...
  if (parent_pid == own_pid) die("parent_pid is equal to own pid!");

  char buf;
  int pfd[2];
  if (pipe(pfd) == -1) die("Failed to create pipe");
...
#include "asan_library_name.h"
  ...
  while ((read(pfd[0], &buf, 1)) < 1) {
    ...
    if (errno != EINTR) die("Failed to read signal from child %d", errno);
  }
  if (buf != 37) die("Didn't get 37 from pipe");
  ...
  while (true) {
    ...
    if (aborted) {
      message("Got signal: %d, child_pid: %d", static_cast<int>(terminated),
              static_cast<int>(child_pid));
      abort_child();
    }
    sleep(1);
