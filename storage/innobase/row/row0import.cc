...
[[nodiscard]] bool row_import_rec_check_old_metadata(const dict_index_t *index,
  ...
  /* 8.0 can upgrade 5.6 data directories where SYS_INDEXES does not yet have
  MERGE_THRESHOLD. Remove this exception in 8.4+, where 5.6 upgrade is gone. */
  const auto is_old_sys_indexes_rec =
      index->id == DICT_INDEXES_ID && n_fields + 1 == index_n_fields;
  ...
  if (n_fields != index_n_fields && !is_old_sys_indexes_rec &&
  ...
  if (rec_old_is_versioned(rec)) {
    ...
    if (!is_valid_row_version(static_cast<uint8_t>(*row_version))) {
      return false;
    }
...
[[nodiscard]] bool row_import_rec_check_comp_metadata(const dict_index_t *index,
  ...
  static_assert(MAX_ROW_VERSION == 64,
  ...
  if (!index->has_instant_cols_or_row_versions()) {
    ...
    if (is_versioned) {
      ...
      row_version = static_cast<uint8_t>(*nulls);
      --nulls;
