...
int runInsertOne(NDBT_Context *ctx, NDBT_Step *step) {
 ...
 * the abort code will handle it, the runLoadTableFail method will then abort
 * and be assumed to be ok, so as long as the node doesn't crash we're passing
 * the test case and also the operation should be aborted.
 *
...
int insertError5083(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  restarter.insertErrorInAllNodes(5083);
  return 0;
}

int clearError5083(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  restarter.insertErrorInAllNodes(0);
  return 0;
}

static int readOneNoCommit(Ndb *pNdb, NdbConnection *pTrans,
...
int runReadOne(NDBT_Context *ctx, NDBT_Step *step) {
...
int runNoCommitAndClose(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  do {
    ...
    // Update
    CHECK(hugoOps.startTransaction(pNdb) == 0);
    for (i = 0; i < 10; i++) CHECK(hugoOps.pkUpdateRecord(pNdb, i) == 0);
    CHECK(hugoOps.execute_NoCommit(pNdb) == 0);
    CHECK(hugoOps.closeTransaction(pNdb) == 0);

    // Delete
    CHECK(hugoOps.startTransaction(pNdb) == 0);
    for (i = 0; i < 10; i++) CHECK(hugoOps.pkDeleteRecord(pNdb, i) == 0);
    CHECK(hugoOps.execute_NoCommit(pNdb) == 0);
    CHECK(hugoOps.closeTransaction(pNdb) == 0);

    // Try to insert, record should already exist
    CHECK(hugoOps.startTransaction(pNdb) == 0);
    for (i = 0; i < 10; i++) CHECK(hugoOps.pkInsertRecord(pNdb, i) == 0);
    CHECK(hugoOps.execute_Commit(pNdb) == 630);
    CHECK(hugoOps.closeTransaction(pNdb) == 0);
...
int runCheckRollbackUpdate(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  do {
    ...
    // Update  record 5
    CHECK(hugoOps.startTransaction(pNdb) == 0);
    CHECK(hugoOps.pkUpdateRecord(pNdb, 1, numRecords, 5) ==
          0);  // Updates value 5
    CHECK(hugoOps.execute_NoCommit(pNdb) == 0);

    // Check record is updated
    CHECK(hugoOps.pkReadRecord(pNdb, 1, numRecords,
  ...
  do {
    ...
    for (Uint32 i = 0; i < 1; i++) {
      ...
      for (j = 0; j < 10; j++) {
        ...
        CHECK(hugoOps.pkReadRecord(pNdb, 5, 10, NdbOperation::LM_Exclusive) ==
              0);
        CHECK(hugoOps.execute_NoCommit(pNdb) == 0);
...
int runMassiveRollback(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int row = 0; row < records; row++) {
    ...
    for (Uint32 i = 0; i < OPS_TOTAL; i += OPS_PER_TRANS) {
      ...
      g_info << "Performed " << (i + OPS_PER_TRANS)
             << " updates on row: " << row << endl;
      if (result != NDBT_OK) {
        break;
      }
...
int runMassiveRollback3(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  HugoOperations hugoOps(*ctx->getTab());
  Ndb *pNdb = GETNDB(step);
...
int runMassiveRollback4(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  HugoOperations hugoOps(*ctx->getTab());
  Ndb *pNdb = GETNDB(step);
...
static TupError f_tup_errors[] = {
    ...
    {NdbOperation::InsertRequest, 4019,
     TupError::TE_REPLICA},  // Alloc rowid error
    {NdbOperation::InsertRequest, 4020,
     TupError::TE_MULTI_OP},  // Size change error
    {NdbOperation::InsertRequest, 4021,
     TupError::TE_DISK},  // Out of disk space
    {NdbOperation::InsertRequest, 4022,
     TupError::TE_OI},  // Tux add error first
...
static int compare(unsigned block, struct ndb_mgm_events *time0,
                   struct ndb_mgm_events *time1) {
  int diff = 0;
  for (int i = 0; i < time0->no_of_events; i++) {
    if (time0->events[i].MemoryUsage.block != block) continue;

    unsigned node = time0->events[i].source_nodeid;

    for (int j = 0; j < time1->no_of_events; j++) {
      if (time1->events[j].MemoryUsage.block != block) continue;

      if (time1->events[j].source_nodeid != node) continue;

      int difference = time1->events[j].MemoryUsage.pages_used -
                       time0->events[i].MemoryUsage.pages_used;
      if (difference > 0 || difference < 0) {
        diff = 1;
        ndbout_c("i: %u, j: %u, before: %u, after: %u", i, j,
                 time0->events[i].MemoryUsage.pages_used,
...
int runTupErrors(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  Ndb *pNdb = GETNDB(step);
  ...
  for (i = 0; i < tab->getNoOfColumns(); i++) {
    if (tab->getColumn(i)->getArrayType() !=
  ...
  for (i = 0; i < (int)l.count; i++) {
    if (DictTabInfo::isOrderedIndex(l.elements[i].type))
      bits |= TupError::TE_OI;
    if (DictTabInfo::isUniqueIndex(l.elements[i].type)) bits |= TupError::TE_UI;
  }
  ...
  for (i = 0; f_tup_errors[i].op != -1; i++) {
    ...
    if ((f_tup_errors[i].bits & bits) != f_tup_errors[i].bits) {
      g_err << "Skipping " << f_tup_errors[i].error << " - req bits: " << hex
            << f_tup_errors[i].bits << " bits: " << hex << bits << endl;
      continue;
    }
...
int runInsertError2(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  HugoOperations hugoOp1(*ctx->getTab());
  Ndb *pNdb = GETNDB(step);
...
int runBug25090(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
...
int runDeleteRead(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
...
int runBug28073(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
...
int runBug20535(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const NdbDictionary::Table *tab = ctx->getTab();
  ...
  if (pTrans->execute(NoCommit) != 0) return NDBT_FAILED;
  ...
  if (pTrans->execute(Commit) != 0) return NDBT_FAILED;
...
int runDDInsertFailUpdateBatch(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbRestarter restarter;
  ...
  for (Uint32 loop = 0; loop < 100; loop++) {
    ...
    /* Create batch with insert op (which will fail due to disk allocation
     * issue) followed by update op on same pk Transaction will abort due to
     * insert failure, and reason should be disk space exhaustion, not any issue
     * with the update.
     */
...
int runBug34348insert(NDBT_Context *ctx, NDBT_Step *step, HugoOperations &ops,
                      int i, bool *rangeFull) {
  ...
  while (result == NDBT_OK) {
    ...
    chk2(ops.execute_Commit(pNdb) == 0 ||
             (code = ops.getNdbError().code) == rangeFullError,
         ops.getNdbError());
    ops.closeTransaction(pNdb);
...
int runBug34348delete(NDBT_Context *ctx, NDBT_Step *step, HugoOperations &ops,
                      int i) {
  Ndb *pNdb = GETNDB(step);
  int result = NDBT_OK;
  while (result == NDBT_OK) {
...
int runBug34348(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (result == NDBT_OK) {
    ...
    while (result == NDBT_OK) {
      ...
      chk2((pTab = pDict->getTable(tabname_bug34348)) != 0,
           pDict->getNdbError());
      break;
    }
    ...
    while (result == NDBT_OK && loop < loops) {
      ...
      if (loop == 0)
        ...
        chk2(rowmaxprev == rowmax,
             "rowmaxprev:" << rowmaxprev << " rowmax:" << rowmax);
      ...
      {
        int i;
        for (i = 0; i < rowmax; i++) BitmaskImpl::set(sz, rowmask, i);
      }

      // random delete until insert succeeds
      while (result == NDBT_OK) {
        int i = myRandom48(rowmax);
        if (!BitmaskImpl::get(sz, rowmask, i)) continue;
...
int runUnlocker(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  g_err << "Unlocker : ";
  g_err << "Loops = " << loops << " Records = " << records
  ...
  while (i++ < loops) {
    ...
    while (r > 0) {
      ...
      check(hugoOps.pkReadRecordLockHandle(ndb, lockHandles, records - r,
                                           batchContents,
                                           (NdbOperation::LockMode)lm) == 0,
            hugoOps);
      ...
      check(hugoOps.pkUnlockRecord(ndb, lockHandles) == 0, hugoOps);
      ...
      if (doubleUnlock) {
        ...
        g_err << "Double unlock, abort option is " << ao << endl;

        /* Failure scenario */
        check(hugoOps.pkUnlockRecord(ndb, lockHandles,
                                     0,     // offset
                                     ~(0),  // NumRecords
                                     ao) == 0,
              hugoOps);

        check(hugoOps.execute_NoCommit(ndb, DefaultAbortOption) != 0, hugoOps);

        /* 417 = Bad operation reference */
        check(hugoOps.getNdbError().code == 417, hugoOps);
      ...
      check(hugoOps.releaseLockHandles(ndb, lockHandles) == 0, hugoOps);
...
int runBug54986(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  Ndb *pNdb = GETNDB(step);
  NdbDictionary::Dictionary *pDict = pNdb->getDictionary();
...
int runBug54944(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const NdbDictionary::Table *pTab = ctx->getTab();
  NdbRestarter res;
...
int runBug59496_scan(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const NdbDictionary::Table *pTab = ctx->getTab();
  NdbRestarter res;
  int rowcount = ctx->getProperty("CHECK_ROWCOUNT", Uint32(0));
  int records = ctx->getNumRecords();
  if (rowcount == 0) records = 0;

  HugoTransactions hugoTrans(*pTab);
  while (!ctx->isTestStopped()) {
    if (hugoTrans.scanReadRecords(pNdb, records, 0, 0,
...
int runBug59496_case1(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbRestarter res;
...
int runBug59496_case2(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  NdbRestarter res;
...
int runTest899(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const NdbDictionary::Table *pTab = ctx->getTab();
  ...
  ndbout_c("count_ok: %d count_failed: %d (899: %d)", count_ok, count_failed,
           count_899);
  delete[] pRow;
...
int runInit899(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  ...
  const NdbDictionary::Table *pTab2 =
      pNdb->getDictionary()->getTable(pTab->getName());
  ...
  for (int i = 0; i < restarter.getNumDbNodes(); i++) {
    if (i & 1) {
      ...
      ndbout_c("Setting slow LCP of table %d on node %d", tableId, nodeId);
...
int runEnd899(NDBT_Context *ctx, NDBT_Step *step) {
  // reset LCP speed
  NdbRestarter restarter;
...
int runVerifyRowCount(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  do {
    ...
    while (scan->nextResult() == 0) rowCountSum += rowCountRA->u_32_value();
    ...
    while (scan->nextResult() == 0) rowScanCount++;
...
int collectEvents(Ndb *ndb, HugoCalculator &calc,
                  const NdbDictionary::Table &tab,
                  Vector<EventInfo> &receivedEvents, int idCol, int updateCol,
  ...
  while (true) {
    ...
    if (res <= 0) {
      ...
      if (!pOp) {
         ...
         */
        if (some_event_data_received && --MaxEmptyPollsAfterData == 0) break;
      }
    }

    {
      while (pOp) {
        bool isDelete =
            (pOp->getEventType() == NdbDictionary::Event::TE_DELETE);
        Vector<NdbRecAttr *> *whichVersion =
            isDelete ? beforeAttrs : afterAttrs;
        ...
        g_err << "# " << receivedEvents.size() << " GCI : " << (gci >> 32)
              << "/" << (gci & 0xffffffff) << " id : " << id
              << " scenario : " << scenario << " optype : " << optype
              << " record : " << recNum << "  ";
        ...
        for (int i = 0; i < (int)whichVersion->size(); i++) {
          ...
          if (!isDelete || tab.getColumn(i)->getPrimaryKey()) {
            NdbRecAttr *ra = (*whichVersion)[i];
            if (calc.verifyRecAttr(recNum, updatesValue, ra) != 0) {
              g_err << "Verify failed on recNum : " << recNum
                    << " with updates value " << updatesValue << " for column "
                    << ra->getColumn()->getAttrId() << endl;
              return NDBT_FAILED;
            }
          }
        }
...
int verifyEvents(const Vector<EventInfo> &receivedEvents,
                 const Vector<ApiEventType> &expectedEvents, int records) {
  ...
  for (Uint32 e = 0; e < receivedEvents.size(); e++) {
    ...
    if (ei.gci != currGci) {
      if (ei.gci < currGci) abort();

      /* Epoch boundary */
      /* At this point, all id counts must be equal */
      for (int i = 0; i < records; i++) {
        if (keys[i] != keys[0]) {
          g_err << "Count for id " << i << " is " << keys[i]
    ...
    if (ei.type != et) {
      g_err << "Expected event of type " << et << " but found " << ei.type
            << " at expectedEvent " << eventIndex << " and event num " << e
            << endl;
      return NDBT_FAILED;
    }
  }
...
int runRefreshTuple(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int a = 0; a < tab.getNoOfColumns(); a++) {
    ...
    if (calc.isIdCol(a)) idCol = a;
    if (calc.isUpdateCol(a)) updateCol = a;
  }

  /* Now execute the event */
  if (pOp->execute()) {
    g_err << "Event operation execution failed : " << pOp->getNdbError()
          << endl;
    return NDBT_FAILED;
  }
  ...
  for (scenario = 0; scenario < 2; scenario++) {
    g_err << "Scenario = " << scenario << " ( Refresh "
          << ((scenario == 0) ? "before" : "after") << " operations )" << endl;
    int optype = 0;
    bool done = false;
    int expectedError = 0;
    do {
      ...
      switch (optype) {
        ...
        case 6: {
          ...
          int anyValue = ((2) << 8) | optype;
          check(hugoOps.pkRefreshRecord(ndb, 0, records, anyValue) == 0,
                hugoOps);
      ...
      if (runVerifyRowCount(ctx, step) != NDBT_OK) return NDBT_FAILED;
  ...
  int rc = collectEvents(ndb, calc, tab, receivedEvents, idCol, updateCol,
                         &eventBeforeRecAttr, &eventAfterRecAttr);
  if (rc == NDBT_OK) {
    rc = verifyEvents(receivedEvents, expectedEvents, records);
  }
...
static int runLeakApiConnectObjects(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  /**
  ...
  for (uint i = 0; i < maxTrans + 1; i++) {
     ...
     */
    if (trans->execute(Commit) != 0 && trans->getNdbError().code != 286) {
...
enum PreRefreshOps { PR_NONE, PR_INSERT, PR_INSERTDELETE, PR_DELETE };
...
const char *opTypeNames[] = {"READ_C", "READ_S", "READ_E", "INSERT",
                             "UPDATE", "WRITE",  "DELETE"};

int runRefreshLocking(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (Uint32 s = 0; s < numScenarios; s++) {
    ...
    switch (scenario.preRefreshOps) {
      ...
      case PR_INSERTDELETE:
        ...
        if (scenario.preRefreshOps == PR_INSERT) break;
        [[fallthrough]];
      case PR_DELETE:
...

    {
      ...
      while (ot < OP_LAST) {
        ...
        switch (ot) {
          case OP_READ_C:
            res =
                hugoOps.pkReadRecord(ndb, 0, 1, NdbOperation::LM_CommittedRead);
            break;
          case OP_READ_S:
        ...
        if ((ot == OP_READ_C) && (scenario.preExist)) {
          if (hugoOps.getNdbError().code == 0) {
            ...
            g_err << "UNEXPECTED : Read committed failed. "
                  << hugoOps.getNdbError() << endl;
            return NDBT_FAILED;
          }
        } else {
          if (hugoOps.getNdbError().code == 0) {
    ...
    if (scenario.preExist) {
      ...
      if (hugoTrans.pkDelRecords(ndb, 0) != 0) {
        g_err << "Delete pre existing failed : " << hugoTrans.getNdbError()
              << endl;
        return NDBT_FAILED;
      }
    }
...
int runBugXXX_init(NDBT_Context *ctx, NDBT_Step *step) { return NDBT_OK; }

int runBugXXX_trans(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter res;
  while (!ctx->isTestStopped()) {
...
int runBugXXX_createIndex(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter res;
  const int loops = ctx->getNumLoops();
...
int runDeleteNdbInFlight(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
...
int runBug16834333(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
...
int runAccCommitOrder(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const NdbDictionary::Table *pTab = ctx->getTab();
  const int loops = ctx->getNumLoops();
  const int records = ctx->getNumRecords();
  ...
  for (int loop = 0; loop < loops; loop++) {
    ...
    if (ctx->getPropertyWait("RUNNING", (Uint32)opsteps)) break;
    g_info << "all op steps running" << endl;

    int mssleep = 10 + ndb_rand() % records;
    if (mssleep > 1000) mssleep = 1000;
    ...
    if (ctx->getPropertyWait("RUNNING", (Uint32)0)) break;
...
int runAccCommitOrderOps(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int loop = 0;
  while (!ctx->isTestStopped()) {
    if (ctx->getPropertyWait("RUN", (Uint32)1)) break;
    ...
    while (ctx->getProperty("RUN", (Uint32)0) == (Uint32)1) {
      ...
      CHK2(
          ops.execute_Commit(pNdb) == 0 || ops.getNdbError().code == 626 ||
...
int runInsertOneTuple(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  const NdbDictionary::Table *table = ctx->getTab();
  HugoOperations hugoOp1(*table);
  Ndb *pNdb = GETNDB(step);
...
int runLockTuple(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  const NdbDictionary::Table *table = ctx->getTab();
  HugoOperations hugoOp1(*table);
  Ndb *pNdb = GETNDB(step);
  ...
  if (!ctx->isTestStopped()) CHECK2(hugoOp1.execute_Commit(pNdb) == 0);
  g_info << "Txn1 commited, ndb " << pNdb << endl;

  if (!ctx->isTestStopped()) CHECK2(hugoOp1.closeTransaction(pNdb) == 0);
  return result;
}

// Read the tuple locked by Txn1, see runLockTuple().
int runReadLockedTuple(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  const NdbDictionary::Table *table = ctx->getTab();
  HugoOperations hugoOp2(*table);
  Ndb *pNdb = GETNDB(step);
  ...
  // Now commit Txn2
  g_info << "Txn2 commits, ndb " << pNdb << ", Ndb to delete " << ndbToDelete
         << endl
         << endl;
...
int testAbortRace(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int r = 0; r < 10; r++) {
    ...
    CHECK2(hugoTrans.scanReadRecords(pNdb, ctx->getNumRecords()) == 0);
 ...
 */
int runCheckLCPStats(NDBT_Context *ctx, NDBT_Step *step) {
  NdbRestarter restarter;
  ...
  Uint32 no_of_replicas =
      mgmd.get_config32(CFG_SECTION_NODE, CFG_DB_NO_REPLICAS);
  ...
  Uint64 end_time = NdbTick_CurrentMillisecond() + (max_wait_seconds * 1000);

  // Read 'Completed LCP' events and sum up the checkpointed records
  while (NdbTick_CurrentMillisecond() < end_time) {
    char buff[512];
    if (!mgmd.get_next_event_line(buff, sizeof(buff), 10 * 1000) == 0) {
      ...
      if (strstr(buff, "Completed LCP")) {
        ...
        sscanf(buff,
               "Node %u: LDM(%u): Completed LCP, #frags = %u #records = %u, "
               "#bytes = %u",
               &node, &ldm, &nfrags, &nRecords, &nBytes);

        g_info << "Node " << node << " ldm " << ldm << " Records " << nRecords
               << endl;
    ...
    for (Uint32 rowId = 0; rowId < numRows; rowId++) {
      ...
      switch (scenario) {
        ...
        case 3: {
          ...
          ndbout_c("RowId %u (%u): Insert, Update, Delete*", rowId,
                   offset + rowId);
          for (Uint32 i = 0; i < 6; i++) {
            CHECK3(hugoOps.pkInsertRecord(pNdb, offset + rowId, 1, 2 * i + 1) ==
                   0);
            CHECK3(hugoOps.pkUpdateRecord(pNdb, offset + rowId, 1, 2 * i + 2) ==
                   0);
      ...
      while ((op = trans->getNextCompletedOperation(op)) != NULL) {
        ndbout_c("Operation %p error %u %s", op, op->getNdbError().code,
...
TESTCASE("PkDirtyRead",
         "Verify that we can insert, dirty read and delete from this table "
         "using PK") {
...
TESTCASE("PkSimpleRead",
         "Verify that we can insert, simple read and delete from this table "
         "using PK") {
...
TESTCASE("PkDelete", "Verify that we can delete from this table using PK") {
  INITIALIZER(runLoadTable);
  STEP(runPkDelete);
  FINALIZER(runClearTable);
}
...
TESTCASE("RollbackDelete", "Test rollback of a no committed delete") {
  INITIALIZER(runLoadTable);
  INITIALIZER(runCheckRollbackDelete);
  FINALIZER(runClearTable2);
}
TESTCASE("RollbackUpdate", "Test rollback of a no committed update") {
  INITIALIZER(runLoadTable);
  INITIALIZER(runCheckRollbackUpdate);
  FINALIZER(runClearTable2);
}
TESTCASE("RollbackDeleteMultiple", "Test rollback of 10 non committed delete") {
  INITIALIZER(runLoadTable);
...
TESTCASE("RollbackNothing", "Test rollback of nothing") {
  INITIALIZER(runLoadTable);
  INITIALIZER(runRollbackNothing);
  FINALIZER(runClearTable2);
}
TESTCASE("MassiveRollback", "Test rollback of 4096 operations") {
  INITIALIZER(runClearTable2);
  INITIALIZER(runMassiveRollback);
  FINALIZER(runClearTable2);
}
TESTCASE("MassiveRollback2", "Test rollback of 4096 operations") {
  INITIALIZER(runClearTable2);
  INITIALIZER(runMassiveRollback2);
  FINALIZER(runClearTable2);
}
TESTCASE("MassiveRollback3", "Test rollback of 4096 operations") {
  INITIALIZER(runClearTable2);
  STEP(runMassiveRollback3);
  STEP(runMassiveRollback3);
  FINALIZER(runClearTable2);
}
TESTCASE("MassiveRollback4", "Test rollback of 4096 operations") {
  INITIALIZER(runClearTable2);
  STEP(runMassiveRollback4);
  STEP(runMassiveRollback4);
  FINALIZER(runClearTable2);
}
TESTCASE("MassiveTransaction", "Test very large insert transaction") {
  INITIALIZER(runLoadTable2);
  FINALIZER(runClearTable2);
}
TESTCASE("TupError", "Verify what happens when we fill the db") {
  INITIALIZER(runTupErrors);
}
TESTCASE("InsertError", "") { INITIALIZER(runInsertError); }
TESTCASE("InsertError2", "") { INITIALIZER(runInsertError2); }
TESTCASE("Fill", "Verify what happens when we fill the db") {
  STEP(runFillTable);
}
TESTCASE("Bug25090", "Verify what happens when we fill the db") {
  STEP(runBug25090);
}
TESTCASE("DeleteRead", "Verify Delete+Read") {
  INITIALIZER(runLoadTable);
  INITIALIZER(runDeleteRead);
  FINALIZER(runClearTable2);
}
TESTCASE("Bug27756", "Verify what happens when we fill the db") {
  STEP(runBug27756);
}
TESTCASE("Bug28073", "Infinite loop in lock queue") { STEP(runBug28073); }
TESTCASE("Bug20535", "Verify what happens when we fill the db") {
  STEP(runBug20535);
}
...
TESTCASE("UnlockBatch", "Test that batched unlock operations work ok") {
  TC_PROPERTY("Batchsize", 33);
  INITIALIZER(runLoadTable);
  STEP(runUnlocker);
  FINALIZER(runClearTable);
}
TESTCASE("DoubleUnlock", "Test that batched unlock operations work ok") {
  TC_PROPERTY("DoubleUnlock", 1);
  INITIALIZER(runLoadTable);
  STEP(runUnlocker);
  FINALIZER(runClearTable);
}
TESTCASE("UnlockUpdateBatch", "Test Unlock mixed with Update") {
...
TESTCASE("RefreshTuple", "Test refreshTuple() operation properties") {
  INITIALIZER(initSubscription);
  INITIALIZER(runRefreshTuple);
  FINALIZER(removeSubscription);
}
TESTCASE("Bug54986", "") { INITIALIZER(runBug54986); }
TESTCASE("Bug54944", "") {
...
TESTCASE("LeakApiConnectObjects", "") { INITIALIZER(runLeakApiConnectObjects); }
TESTCASE("RefreshLocking", "Test Refresh locking properties") {
...
TESTCASE("Bug16834333", "") { INITIALIZER(runBug16834333); }
TESTCASE("DeleteNdbInFlight", "") { INITIALIZER(runDeleteNdbInFlight); }
...
TESTCASE("DeleteNdbWhilePoll",
         "Delete an Ndb while it(trp_clnt) is in poll queue. Will crash the "
         "test, and thus not to be run in a regular test") {
...
TESTCASE("AbortRace", "Test race between ABORT and PREPARE processing at LDM") {
  INITIALIZER(runLoadTable);
  STEP(testAbortRace);
  FINALIZER(runClearTable);
}
