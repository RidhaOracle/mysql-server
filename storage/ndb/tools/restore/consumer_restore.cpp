...
bool BackupRestore::init(Uint32 tableChangesMask) {
  ...
  for (Uint32 i = 0; i < m_parallelism; i++) {
    ...
    m_callback[i].logRestore = false;
    m_callback[i].retries = 0;
    m_callback[i].error_code = 0;
    m_callback[i].fragId = 0;
    m_callback[i].n_bytes = 0;
    m_callback[i].leKeyHash = 0;
    m_callback[i].next_waiter = nullptr;
    m_callback[i].last_waiter = nullptr;
    if (i > 0) m_callback[i - 1].next = &(m_callback[i]);
  }
  m_callback[m_parallelism - 1].next = 0;

  m_transScheduler.init(m_parallelism);
...
bool BackupRestore::table_compatible_check(TableS &tableS) {
   ...
   */
  {
    ...
    for (unsigned i = 0; i < index_list.count; i++) {
      ...
      if (strncmp(index_name, PK_MAPPING_IDX_NAME,
                  strlen(PK_MAPPING_IDX_NAME)) == 0) {
        /* This index is generated by another ndb_restore
         * thread or process, not relevant for this check
         * and may not be openable
         */
        continue;
      }
  ...
  for (int i = 0; i < tableS.m_dictTable->getNoOfColumns(); i++) {
    ...
    if (col_in_kernel == 0) {
      ...
      restoreLogger.log_info(
          "Column in backup (%s.%s) missing in DB."
  ...
  for (int i = 0; i < tab->getNoOfColumns(); i++) {
    ...
    if (col_in_backup == 0) {
      ...
      if ((m_tableChangesMask & TCM_EXCLUDE_MISSING_COLUMNS) == 0) {
        restoreLogger.log_error(
            "Missing column(%s.%s) in backup and "
            "exclude-missing-columns not specified",
      ...
      /**
       * only nullable or defaulted non primary key columns can be missing from
       * backup
       *
       */
      ...
      restoreLogger.log_info(
          ...
          " Will be set to %s.",
          tableS.m_dictTable->getName(), col_in_kernel->getName(),
          ((col_in_kernel->getDefaultValue() == NULL) ? "Null"
                                                      : "Default value"));
    }
  }
  ...
  if (pk_extended_in_kernel) {
    ...
    if (table_has_blob_parts) {
       ...
       */
      restoreLogger.log_info(
          "Table %s has Blob/Text columns with part tables "
  ...
  if (pk_extended_in_kernel || !full_pk_present_in_kernel) {
  ...
  for (int i = 0; i < tableS.m_dictTable->getNoOfColumns(); i++) {
    ...
    if (attr_desc->m_exclude) continue;

    const NDBCOL *col_in_kernel = tab->getColumn(attr_desc->attrId);
    const NDBCOL *col_in_backup = tableS.m_dictTable->getColumn(i);

    if (column_compatible_check(tablename, col_in_backup, col_in_kernel)) {
      continue;
    }
    ...
    const bool col_in_kernel_pk = col_in_kernel->getPrimaryKey();
    attrCheckCompatFunc =
        get_attr_check_compatability(type_in_backup, type_in_kernel);
    AttrConvType compat =
        (attrCheckCompatFunc == NULL
             ? ACT_UNSUPPORTED
    ...
    switch (compat) {
      ...
      case ACT_STAGING_LOSSY:
        ...
        break;
      default:
        restoreLogger.log_error("internal error: illegal value of compat = %u",
                                compat);
        assert(false);
        return false;
    };
    ...
    if (type_in_backup == NDBCOL::Char || type_in_backup == NDBCOL::Binary ||
        type_in_backup == NDBCOL::Bit || type_in_backup == NDBCOL::Varchar ||
        type_in_backup == NDBCOL::Longvarchar ||
        type_in_backup == NDBCOL::Varbinary ||
        type_in_backup == NDBCOL::Longvarbinary) {
      unsigned int size =
          sizeof(struct char_n_padding_struct) + m_attrSize * m_arraySize;
      struct char_n_padding_struct *s =
          (struct char_n_padding_struct *)malloc(size + 2);
      if (!s) {
      ...
      struct char_n_padding_struct *s =
          (struct char_n_padding_struct *)malloc(size);
      if (!s) {
        restoreLogger.log_error("No more memory available!");
        return false;
      }
    ...
    if (tab->getTablespace(&ts_id)) {
      ...
      if (strlen(ts_name) == 0) {
        restoreLogger.log_error(
            ...
            tablename, ts_id, dict->getNdbError().code,
            dict->getNdbError().message);
        return false;
      }
...
bool BackupRestore::handle_index_stat_tables() {
  if (!m_restore_meta) return true;
...
bool BackupRestore::table(const TableS &table) {
   ...
   * Ignore blob tables
   */
  if (match_blob(name) >= 0) return true;
  ...
  if (m_restore_meta) {
    ...
    if (part_bal == NdbDictionary::Object::PartitionBalance_ForRPByLDM) {
      ...
      if (!copy.getDefaultNoPartitionsFlag()) {
        ...
        restoreLogger.log_info(
            "Setting %s to specific partition balance with "
            "%u fragments.",
    ...
    if (copy.getFragmentType() == NdbDictionary::Object::HashMapPartition) {
      ...
      */
      ...
      copy.setFragmentCount(get_no_fragments(copy.getMaxRows(), no_nodes));
    ...
    */
    ...
    if (tab->getMaxRows() != 0 && table.getNoOfRecords() > copy.getMaxRows()) {
  ...
  if (m_restore_meta) {
    if (tab->getNoOfAutoIncrementColumns()) {
      ...
      for (int retries = 0; retries < MAX_RETRIES; retries++) {
        int res = m_ndb->setAutoIncrementValue(tab, Uint64(1), false);
        if (res == 0) {
          break;
        }
        ...
        restoreLogger.log_error(
            ...
            table_name.c_str(), m_ndb->getNdbError().code,
            m_ndb->getNdbError().message);
        return false;
      }
    }
...
bool BackupRestore::fk(Uint32 type, const void *ptr) {
  ...
  switch (type) {
    case DictTabInfo::ForeignKey: {
      ...
      if (!dissect_table_name(fk_ptr->getChildTable(), db_name, dummy,
                              table_name))
        return false;
      for (unsigned i = 0; i < m_new_tables.size(); i++) {
        if (m_new_tables[i] == NULL) continue;
      ...
      if (child) {
        ...
        if (!dissect_table_name(fk_ptr->getParentTable(), db_name, dummy,
                                table_name))
          return false;
        m_ndb->setDatabaseName(db_name.c_str());
        NdbDictionary::Dictionary *dict = m_ndb->getDictionary();
        ...
        if (parent == 0) {
          restoreLogger.log_error(
              ...
              fk_ptr->getName(), db_name.c_str(), table_name.c_str(),
              dict->getNdbError().code, dict->getNdbError().message);
          return false;
        }
        ...
        if (m_disable_indexes) {
          ...
          if (dict->getForeignKey(fk, fullname) == 0) {
            ...
            restoreLogger.log_info("Successfully dropped foreign key %s",
                                   fkname);
          }
        }
      }
...
bool BackupRestore::endOfTables() {
  ...
  for (unsigned i = 0; i < m_indexes.size(); i++) {
    ...
    BaseString db_name, schema_name, table_name;
    if (!dissect_table_name(indtab.m_primaryTable.c_str(), db_name, schema_name,
                            table_name)) {
      return false;
    }
    check_rewrite_database(db_name);
...
bool BackupRestore::endOfTablesFK() {
  ...
  for (unsigned i = 0; i < m_fks.size(); i++) {
    ...
    char cInfo[512] = "?";
    {
      ...
      if (!dissect_table_name(fkinfo.getParentTable(), db_name, dummy2,
                              table_name))
        return false;
      m_ndb->setDatabaseName(db_name.c_str());
      ...
      if (pTab == 0) {
        restoreLogger.log_error(
            "Foreign key %s"
            " parent table %s.%s not found: %u: %s",
            fkname, db_name.c_str(), table_name.c_str(),
            dict->getNdbError().code, dict->getNdbError().message);
        return false;
      }
      if (fkinfo.getParentIndex() != 0) {
        BaseString dummy1, dummy2, index_name;
        if (!dissect_index_name(fkinfo.getParentIndex(), dummy1, dummy2,
                                index_name))
          return false;
      ...
      BaseString::snprintf(pInfo, sizeof(pInfo), "%s.%s.%s", db_name.c_str(),
                           table_name.c_str(), pInd ? pInd->getName() : "PK");
    }
    {
      BaseString db_name, dummy2, table_name;
      if (!dissect_table_name(fkinfo.getChildTable(), db_name, dummy2,
                              table_name))
        return false;
      m_ndb->setDatabaseName(db_name.c_str());
      ...
      if (fkinfo.getChildIndex() != 0) {
        BaseString dummy1, dummy2, index_name;
        if (!dissect_index_name(fkinfo.getChildIndex(), dummy1, dummy2,
                                index_name))
          return false;
      ...
      BaseString::snprintf(cInfo, sizeof(cInfo), "%s.%s.%s", db_name.c_str(),
                           table_name.c_str(), cInd ? cInd->getName() : "PK");
    }
...
void BackupRestore::update_next_auto_val(Uint32 orig_table_id,
                                         Uint64 next_val) {
...
bool BackupRestore::get_fatal_error() { return m_fatal_error; }
  ...
  while (cb->retries < MAX_RETRIES) {
    ...
    for (int j = 0; j < 2; j++) {
      for (int i = 0; i < tup.getNoOfAttributes(); i++) {
        ...
        if (attr_desc->convertFunc) {
          if ((col_pk_in_kernel && j == 0) || (j == 1 && !attr_data->null)) {
            bool truncated = true;  // assume data truncation until overridden
            dataPtr = (char *)attr_desc->convertFunc(
                dataPtr, attr_desc->parameter, truncated);
            if (!dataPtr) {
              const char *tabname = tup.getTable()->m_dictTable->getName();
              restoreLogger.log_error(
                  "Error: Convert data failed when restoring tuples!"
                  " Data part, table %s",
                  tabname);
              set_fatal_error(true);
            ...
            if (truncated) {
              ...
              // restoreLogger.log_error("======  data truncation detected for
              // column: "
        ...
        if (ret < 0) {
          ndbout_c("Column: %d type %d %d %d %d", i,
                   attr_desc->m_column->getType(), size, arraySize, length);
          break;
        }
        n_bytes += length;
      }
    ...
    cb->n_bytes = n_bytes;
    cb->connection->executeAsynchPrepare(NdbTransaction::Commit, &callback, cb);
    m_transactions++;
    return;
  }
  restoreLogger.log_error(
      "Retried transaction %u times.\nLast error %u %s"
...
void BackupRestore::tuple_SYSTAB_0(restore_callback_t *cb, const TableS &tab) {
  const TupleS &tup = cb->tup;
  Uint32 syskey;
  Uint64 nextid;
...
void BackupRestore::cback(int result, restore_callback_t *cb) {
  ...
  if (m_error_insert == NDB_RESTORE_ERROR_INSERT_FAIL_RESTORE_TUPLE &&
      m_transactions > 10) {
    restoreLogger.log_error(
        "Error insert NDB_RESTORE_ERROR_INSERT_FAIL_RESTORE_TUPLE");
    m_error_insert = 0;
    set_fatal_error(true);
  }
#endif
...
void BackupRestore::data_free() {
  if (!m_restore) return;

  /**
   * Iterator wants to free up data it has passed to us
   * in previous invocations.
   * In this call we flush all work so that we do not
   * retain any references to the previously passed
   * data.
   */
...
void BackupRestore::TransScheduler::init(Uint32 maxTransactions) {
  ...
  constexpr Uint32 LoadFactor = 3;
  hashTable.fill(LoadFactor * maxTransactions, nullptr);
}

/**
 * findActiveTransForRow
 *
 * Lookup the ActiveTransaction set for a transaction active on
 * the passed transaction's row and return it.
 *
 * Returns nullptr if not found.
 */
restore_callback_t *BackupRestore::TransScheduler::findActiveTransForRow(
    restore_callback_t *trans) {
  require(trans->logRestore);

  const Uint32 bucketNum = (trans->leKeyHash) % hashTable.size();
  restore_callback_t *activeTrans = hashTable[bucketNum];

  while (activeTrans != nullptr) {
    if (activeTrans->leKeyHash == trans->leKeyHash) {
      break;
    }
    activeTrans = activeTrans->next;
  }
 ...
 * insertActiveTrans
 *
 * Insert the supplied transaction into the ActiveTransactions
 * set, keyed by row.
 *
 * Must be no transaction for the row already in the set.
 */
void BackupRestore::TransScheduler::insertActiveTrans(
    restore_callback_t *trans) {
  require(trans->logRestore);
  assert(findActiveTransForRow(trans) == nullptr);

  const Uint32 bucketNum = (trans->leKeyHash) % hashTable.size();
  restore_callback_t *firstInBucket = hashTable[bucketNum];

  /* Add transaction to start of bucket */
  trans->next = firstInBucket;
  hashTable[bucketNum] = trans;
}

/**
 * removeActiveTrans
 *
 * Remove the supplied transaction from the ActiveTransactions
 * set
 *
 * The transaction must be in the set
 */
void BackupRestore::TransScheduler::removeActiveTrans(
    restore_callback_t *trans) {
  require(trans->logRestore);

  const Uint32 bucketNum = (trans->leKeyHash) % hashTable.size();
  restore_callback_t *activeTrans = hashTable[bucketNum];
  restore_callback_t *prevActiveTrans = nullptr;

  while (activeTrans != nullptr) {
    if (activeTrans == trans) {
      /* Found it, remove it */
      if (prevActiveTrans == nullptr) {
        /* First in bucket */
        hashTable[bucketNum] = activeTrans->next;
      } else {
        prevActiveTrans->next = activeTrans->next;
      }
      activeTrans->next = nullptr;
      return;
    }
    prevActiveTrans = activeTrans;
    activeTrans = activeTrans->next;
  }

  /* Transaction was not in bucket */
  require(false);
}
 ...
 * linkWaitingTrans
 *
 * Link a transaction to the end of an ActiveTransaction's
 * list of waiting transactions
 *
 * Maintains next_waiter and last_waiter ptrs
 */
void BackupRestore::TransScheduler::linkWaitingTrans(
    restore_callback_t *activeTrans, restore_callback_t *waitingTrans) {
  /* Link waiting trans to activeTrans */
  if (activeTrans->next_waiter == nullptr) {
    /* First waiter */
    require(activeTrans->last_waiter == nullptr);
    activeTrans->next_waiter = activeTrans->last_waiter = waitingTrans;
  } else {
    /* Link to end of list of waiters to preserve order */
    require(activeTrans->last_waiter);
    activeTrans->last_waiter->next_waiter = waitingTrans;
    activeTrans->last_waiter = waitingTrans;
  }
}

/**
 * unlinkAllWaitingTrans
 *
 * Remove all waiting transaction(s) from the given ActiveTransaction,
 * returning the first waiting transaction (head of list of waiters)
 */
restore_callback_t *BackupRestore::TransScheduler::unlinkAllWaitingTrans(
    restore_callback_t *completedTrans) {
  /* Remove and return any waiting transaction from completed transaction */
  restore_callback_t *next_waiter = completedTrans->next_waiter;
  if (next_waiter) {
    require(completedTrans->last_waiter);
    if (completedTrans->last_waiter != next_waiter) {
      /* Update first waiter's link to last waiter */
      next_waiter->last_waiter = completedTrans->last_waiter;
    }
    completedTrans->next_waiter = completedTrans->last_waiter = nullptr;
  } else {
    require(completedTrans->last_waiter == nullptr);
  }

  return next_waiter;
}

bool BackupRestore::TransScheduler::requestRowAccess(
    restore_callback_t *newTrans) {
  require(newTrans->logRestore);

  newTrans->next = nullptr;
  newTrans->next_waiter = nullptr;
  newTrans->last_waiter = nullptr;

  restore_callback_t *activeTransOnRow = findActiveTransForRow(newTrans);
  if (!activeTransOnRow) {
    insertActiveTrans(newTrans);
    return true;
  }

  /* Have existing transaction operating on row, link
   * new transaction as waiter
   */
  linkWaitingTrans(activeTransOnRow, newTrans);
...
restore_callback_t *BackupRestore::TransScheduler::notifyRowAccessComplete(
    restore_callback_t *completedTrans) {
  require(completedTrans->logRestore);

  /* Unlink all waiting transaction(s) (if any) */
  restore_callback_t *waitingTrans = unlinkAllWaitingTrans(completedTrans);

  /* Remove completed transaction from ActiveTransactions set */
  removeActiveTrans(completedTrans);

  if (waitingTrans) {
    /* Add waiting transaction (with any linked waiter(s)
     * to ActiveTransaction set - it is active now
     */
    insertActiveTrans(waitingTrans);

    /* Indicate caller should execute first waiter now */
    return waitingTrans;
  }
...
bool BackupRestore::tryCreatePkMappingIndex(TableS *table,
  ...
  if (ndbtab == NULL) {
    restoreLogger.log_error("Failed to find table %s in DB.  Error : %u %s.",
                            table->getTableName(), dict->getNdbError().code,
                            dict->getNdbError().message);
    return false;
  }
  ...
  for (int i = 0; i < table->getNoOfAttributes(); i++) {
    ...
    if (attrDesc->m_column->getPrimaryKey()) {
      ...
      if (col != NULL) {
        restoreLogger.log_info(
            "Adding column (%s) DB(%s) to "
            "PK mapping index for table %s.",
            attrDesc->m_column->getName(), col->getName(),
            table->getTableName());
  ...
  if (oldPkColsAvailable == 0) {
    restoreLogger.log_error(
        "Table %s has update or delete backup log "
  ...
  if (dict->createIndex(idx) == 0) {
    ...
    restoreLogger.log_info(
        "Remember to run ndb_restore --rebuild-indexes "
  ...
  */
  ...
  if (createError.code == 721) {
    ...
    restoreLogger.log_error(
        "Failed to create pk mapping index on "
        "table %s %u %s.",
        table->getTableName(), createError.code, createError.message);
    return false;
  }
}

bool BackupRestore::getPkMappingIndex(TableS *table) {
  /**
  ...
  /* Set database, schema */
  BaseString db_name, schema_name, table_name;
  if (!dissect_table_name(table->getTableName(), db_name, schema_name,
                          table_name)) {
  ...
  do {
    dbIdx = dict->getIndex(PK_MAPPING_IDX_NAME, short_table_name);
    ...
    if (dbIdx) {
      ...
      if (getErr.code == 4243) {
         ...
         */
        if (!tryCreatePkMappingIndex(table, short_table_name)) {
          /* Hard failure */
          return false;
        }
        retry_count = 0;
        ...
        restoreLogger.log_error(
            ...
            "table %s %u %s.",
            table->getTableName(), getErr.code, getErr.message);
        return false;
      }
    }
  ...
  restoreLogger.log_error(
      "Failure to lookup / create PK mapping "
      "index after %u attempts.",
      MAX_RETRIES);
  return false;
}

bool BackupRestore::dropPkMappingIndex(const TableS *table) {
  const char *tablename = table->getTableName();
  ...
  while (!dropped && attempts--) {
    dict->dropIndex(PK_MAPPING_IDX_NAME, table_name.c_str());
    const NdbError dropErr = dict->getNdbError();
    switch (dropErr.status) {
      case NdbError::Success:
        restoreLogger.log_info("Dropped PK mapping index on %s.", tablename);
        dropped = true;
        break;
      case NdbError::TemporaryError:
        restoreLogger.log_error("Temporary error: %u %s.", dropErr.code,
                                dropErr.message);
        NdbSleep_MilliSleep(100 + attempts * 300);
        continue;
      case NdbError::PermanentError:
        if (dropErr.code == 723 || dropErr.code == 4243) {
      ...
      default:
        restoreLogger.log_error("Error dropping mapping index on %s %u %s",
                                tablename, dropErr.code, dropErr.message);
        return false;
    }
  }
...
static Uint32 get_part_id(const NdbDictionary::Table *table,
  ...
  if (table->getLinearFlag()) {
    ...
    if (part_id >= no_frags) part_id = hash_value & (mask >> 1);
    return part_id;
  } else
    return (hash_value % no_frags);
}

static void callback_logentry(int result, NdbTransaction *trans,
                              void *aObject) {
  restore_callback_t *cb = (restore_callback_t *)aObject;
  (cb->restore)->cback_logentry(result, cb);
}

bool BackupRestore::logEntry(const LogEntry &le) {
  if (!m_restore) return true;
  ...
  /* Get or wait for available transaction */
  while (m_free_callback == nullptr) {
    assert(m_transactions == m_parallelism);
    // send-poll all transactions
    // close transaction is done in callback
    m_ndb->sendPollNdb(3000);
  ...
  cb->retries = 0;
  cb->le = le;  // Copy of LogEntry object
  cb->logRestore = true;
  cb->leKeyHash = 0;
  cb->next_waiter = nullptr;
  cb->last_waiter = nullptr;

  /* Define operation */
  logEntry_a(cb, false /* not a retry */);

  return (!get_fatal_error());
}

void BackupRestore::logEntry_a(restore_callback_t *cb, bool retry) {
...
retry:
#ifdef ERROR_INSERT
  if (m_error_insert == NDB_RESTORE_ERROR_INSERT_FAIL_REPLAY_LOG &&
      m_log_prepare_count++ == 25) {
    restoreLogger.log_error(
        "Error insert NDB_RESTORE_ERROR_INSERT_FAIL_REPLAY_LOG");
    m_error_insert = 0;
  ...
  if (unlikely(use_mapping_idx)) {
    /* UI access */
    op = trans->getNdbIndexOperation(tup.m_table->m_pk_index, table);
  } else {
    /* Normal pk access */
  ...
  /* Values for calculating key hash */
  uint64 keyHashN1 = 0;
  uint64 keyHashN2 = 4;

  Bitmask<MAXNROFATTRIBUTESINWORDS> hashedKeys;
  ...
  for (Uint32 pass = 0; pass < 2; pass++)  // Keys then Values
  {
    ...
    for (Uint32 i = 0; i < tup.size(); i++) {
      ...
      if (pass == 0 && col_pk_in_backup &&
          !hashedKeys.get(attr->Desc->attrId)) {
        /* Add to key hash calculation */
        hashedKeys.set(attr->Desc->attrId);

        const Uint32 maxLen = attr->Desc->getSizeInBytes();
        Uint32 lb, len;
        const bool ok = NdbSqlUtil::get_var_length(
            attr->Desc->m_column->getType(), dataPtr, maxLen, lb, len);
        require(ok);
        CHARSET_INFO *cs = attr->Desc->m_column->getCharset();
        if (cs == nullptr) cs = &my_charset_bin;

        (*cs->coll->hash_sort)(cs, (const unsigned char *)dataPtr + lb, len,
                               &keyHashN1, &keyHashN2);
      }

      if (attr->Desc->m_exclude) continue;
      ...
      if (unlikely(!col_pk_in_backup && col_pk_in_kernel)) {
        if (unlikely(tup.m_type == LogEntry::LE_UPDATE)) {
          if ((m_tableChangesMask & TCM_IGNORE_EXTENDED_PK_UPDATES) != 0) {
             ...
             */
            restoreLogger.log_error(
                "Error : Primary key remapping failed "
      ...
      {
        bool truncated = true;  // assume data truncation until overridden
        dataPtr = (char *)attr->Desc->convertFunc(
            dataPtr, attr->Desc->parameter, truncated);
        if (!dataPtr) {
  ...
  /* Track callback object being in use */
  m_transactions++;

  /* Calculate + store hash of tableid + primary key */
  const Uint32 keyHash =
      (cb->le.m_table->getTableId() ^ keyHashN1 ^ (keyHashN1 >> 32));

  if (retry) {
    /* Sanity check */
    require(keyHash == cb->leKeyHash);
  } else {
    cb->leKeyHash = keyHash;
  }

  /**
   * Check if we can execute now
   *
   * If this is a retry then the transaction is already permitted
   * so can definitely execute now
   *
   * Otherwise this is a new transaction and we must request
   * permission to execute an operation on the row.
   *
   * If there is no existing active transaction, execute now.
   * If there is an existing active transaction then this one
   * will be queued behind it, do nothing now.
   */
  if (likely(retry || m_transScheduler.requestRowAccess(cb))) {
    /* Do it now */
    logEntry_b(cb);
  } else {
    /* Need to wait for existing active transaction on the same row */
  }

  return;
}

void BackupRestore::logEntry_b(restore_callback_t *cb) {
  /* Make defined transaction + op available for later send + execute */
  NdbTransaction *trans = cb->connection;
  trans->executeAsynchPrepare(NdbTransaction::Commit, &callback_logentry, cb);
  return;
}
...
void BackupRestore::cback_logentry(int result, restore_callback_t *cb) {
  ...
  require(outcome == DONE);
  {
     ...
     * execute any waiting transaction on the same
     * row now.
     */
    restore_callback_t *waiting_trans =
        m_transScheduler.notifyRowAccessComplete(cb);
    ...
    if (waiting_trans) {
      /**
       * Another transaction on the same row was waiting
       * Make it available to send/execute now
       */
      logEntry_b(waiting_trans);
    } else {
      /* Trans completed with no waiter */
    }
  }
